...
param(
    ...
    [ValidateSet('all', 'high', 'rls', 'secrets', 'clientauth', 'csrf', 'authmiddleware', 'headers', 'envvalidation')]
    [string[]]$Checks = @('all')
)
...
$highChecks = @('rls', 'secrets', 'clientauth')
$mediumChecks = @('csrf', 'authmiddleware', 'headers')
$lowChecks = @('envvalidation')
...
Write-Output "`n=== GUARDSCAN SUMMARY ==="
