...
# foreach-as-expression instead of "$arr += [ordered]@{...}": the latter triggers a
# Dynamic-Binder bug in PowerShell 5.1 with nested collection expressions inside an
# [ordered] hashtable literal ("argument types do not match",
# PSEnumerableBinder.MaybeDebase) - discovered during testing.
$symbolResults = @(
    foreach ($sym in $Symbols) {
        [ordered]@{
            symbol   = $sym
...
foreach ($sr in $symbolResults) {
    Write-Output "  Symbol '$($sr.symbol)': $($sr.hitCount) hits$(if ($sr.capped) { ' (capped at 200)' })"
}
