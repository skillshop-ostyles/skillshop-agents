...
function Split-IdentifierToTerms([string]$ident) {
    # -creplace (case-SENSITIVE) is mandatory here: the normal -replace is
    # case-insensitive in PowerShell, causing [a-z0-9] and [A-Z] to both match every
    # letter and wrongly insert the CamelCase boundary between EVERY
    # character pair (e.g. "createCustomer" -> "c re at eC...").
...
$outTermsFinal = @(
    foreach ($t in $outTerms) {
        # Do NOT wrap samples with @(...): $t.samples is already a
        # List[object] - wrapping it again with @() as a hashtable value inside
        # a foreach-in-@() block triggers the same PSEnumerableBinder-Bug
        # as in sprint 3 (+= with nested collection), only this time
        # with direct assignment. Direct assignment of the already-enumerable
        # list bypasses the bug.
        [ordered]@{
            term      = $t.term
...
Write-Output "  Scanned files: $($allFiles.Count)"
Write-Output "  Raw identifiers total: $totalIdentifiers"
Write-Output "  Terms (after MinFrequency $MinFrequency): $($allTerms.Count)"
Write-Output "  Capped at TopN ${TopN}: $truncated"
