...
function Get-Context($lines, $idx) {
    # [string]-Cast: Get-Content-Zeilen tragen PowerShell-ETS-Metadaten (PSPath etc.)
    # am Objekt, die ConvertTo-Json sonst mitserialisiert statt nur den Zeileninhalt.
    $result = @()
...
# 6 Muster-Familien (Grep-Niveau, Simplicity First - siehe Sprint 03 fuer dieselbe
# Design-Entscheidung bei ref-scan.ps1).
...
Write-Output "  Kandidaten gesamt: $($candidateList.Count)$(if ($truncated) { " (gekuerzt auf $MaxCandidates)" })"
