...
# -- File candidates: not referenced by any other file, no entry-point pattern --
$fileCandidates = @(
    foreach ($f in $allFiles) {
        $relPath = $fileRelPaths[$f.FullName]
...
Write-Output "  File candidates (never referenced, no entry point): $($fileCandidates.Count)"
