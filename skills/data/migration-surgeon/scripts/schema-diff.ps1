...
if (-not (Test-Path -LiteralPath $OldSchema)) {
    Write-Error "OldSchema does not exist: $OldSchema"
    exit 1
}
if (-not (Test-Path -LiteralPath $NewSchema)) {
    Write-Error "NewSchema does not exist: $NewSchema"
    exit 1
}
...
if ($oldFormat -ne $newFormat) {
    Write-Error "Format mix not supported: OldSchema detected as '$oldFormat', NewSchema as '$newFormat'. Both states must use the same format."
    exit 1
}
$detectedFormat = $oldFormat

# -- Helper functions: parenthesis-depth-aware splitting (NUMERIC(10,2) must not
#     be mistaken as a column separator) --
...
# -- SQL parsing (deliberately pragmatic, no full SQL parser - simplicity first) --
function Parse-SqlSchema([string]$text) {
    ...
    foreach ($m in [regex]::Matches($text, $createTablePattern)) {
        ...
        foreach ($rawPart in $parts) {
            ...
            if ($part -match '(?i)^(PRIMARY\s+KEY|FOREIGN\s+KEY|UNIQUE|CONSTRAINT|CHECK)\b') {
                continue # Table-wide constraints - not individually modeled for the pragmatic diff
            }
    ...
    # Statements that are not CREATE TABLE / CREATE INDEX, mark as unparsed
    # (Triggers, Views, Functions, ALTER TABLE - intentionally not modeled).
...
Write-Output "  Changes: $($changes.Count) (Tables +$($summary.tablesAdded)/-$($summary.tablesRemoved), Columns +$($summary.columnsAdded)/-$($summary.columnsRemoved)/~$($summary.columnsChanged), Indexes +$($summary.indexesAdded)/-$($summary.indexesRemoved))"
Write-Output "  Rename-Kandidaten: $($renameCandidates.Count)"
if ($unparsedAll.Count -gt 0) {     Write-Output "  WARNING: $($unparsedAll.Count) unparsed statements - check manually." }
