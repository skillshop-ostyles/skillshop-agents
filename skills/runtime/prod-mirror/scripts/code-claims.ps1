...
# Without a (trimmed) line-start anchor, "get("/"post(" etc. also matches
# generic method calls like "request.headers.get(...)" or "cache.get(...)"
# - actual route registrations (app.get(...), router.post(...)) almost always
# stand as their own statement at line start, sub-expressions like "const x =
# foo.headers.get(...)" do not. Found during acceptance run against dreamzzz-api_vs
# (27 false positive hits on Headers.get() etc.).
