...
# --- 2. Projekt-Metadaten: Test-Runner + Test-Kommando ---
$testRunner = $null
$testCommand = $null
...
# --- 4. Entry-Points (Orientierung, kein Anspruch auf Vollstaendigkeit) ---
...
foreach ($k in $runtimes.Keys) { Write-Output "  Runtime $k`: $(if ($runtimes[$k]) { $runtimes[$k] } else { 'nicht gefunden' })" }
Write-Output "  Test-Runner: $(if ($testRunner) { $testRunner } else { 'unbekannt' })"
if ($gitInfo) {
    ...
    Write-Output '  Git: kein Repo'
}
Write-Output "  Entry-Points: $(if ($entryPoints.Count -gt 0) { $entryPoints -join ', ' } else { 'keine gefunden' })"
