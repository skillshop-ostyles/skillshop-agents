...
# Breiter als die im Sprint-File genannte Vergleichs-/Zuweisungs-Form
# ([=<>]\s*['"]?20\d{2}): der Testplan verlangt ausdruecklich, dass auch ein
# freistehendes Copyright-Jahr als Fund auftaucht, damit die LLM-Analyse es
# aktiv als Fehlalarm aussortieren kann (§ 6.1) - eine reine Vergleichs-Regex
# wuerde "Copyright 2019" nie treffen. Deshalb: jede wortgrenzen-genaue
# Jahreszahl 2015-2099, nicht nur in Vergleichs-/Zuweisungs-Kontext.
...
foreach ($f in $allFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        # --- 1. Datums-Literale (nicht in Changelog/Doku-Dateien) ---
        if (-not $isChangelog) {
        ...
        # --- 3. Provisorien-Marker (mit Blame-Alter) ---
