...
if (-not $Author -or $Author.Count -eq 0) {
    Write-Error "-Author is required (except with -ListAuthors). Use -ListAuthors to see candidates."
    exit 1
}
...
if ($commits.Count -eq 0) {
    Write-Error "No commits found for author. Use -ListAuthors to see valid candidates."
    exit 1
}
