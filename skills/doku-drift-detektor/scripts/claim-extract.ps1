...
# --- Doku-Dateien sammeln: README*/CONTRIBUTING*/*.md in der Wurzel + docs/**/*.md rekursiv ---
$rootFiles = @(
...
foreach ($docFile in $docFileInfos) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        # --- Backtick-Tokens: Pfad / Config / Symbol ---
...
# --- Zaehlung je Typ: vollstaendig, unabhaengig von Kappung ---
$countsByType = [ordered]@{ path = 0; command = 0; config = 0; endpoint = 0; version = 0; symbol = 0 }
foreach ($c in $claimMap.Values) { $countsByType[$c.type] += 1 }

# --- Auswahl fuer Ausgabe: bei Kappung command/path priorisieren ---
