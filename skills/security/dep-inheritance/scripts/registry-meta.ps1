...
function Get-NpmMeta($name) {
    ...
    if ($npmCmd) {
        try {
            ...
            # falls through to REST fallback
        }
    }
    try {
        ...
        return @{ meta = $null; metaError = "npm registry unreachable or package unknown: $($_.Exception.Message)" }
    }
}
...
function Get-PypiMeta($name) {
    try {
        ...
        return @{ meta = $null; metaError = "PyPI unreachable or package unknown: $($_.Exception.Message)" }
    }
}

# EVERY error (offline, 404, timeout) -> meta: null + metaError, never abort
# (offline fallback is mandatory per sprint file).
$results = @(
...
foreach ($r in $results) {
    if ($r.meta) {
        ...
        Write-Output "  $($r.name): no metadata ($($r.metaError))"
    }
}
