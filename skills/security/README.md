...
|--|--|--|--|
| [config-cartographer](../security/config-cartographer/) | /config-cartographer | Both | Map a system's full configuration surface: every env var, setting, flag - where it is defined versus where it is read. Reports read-but-never-defined (crash candidates), defined-but-never-read (orphans), and divergent defaults. **Never outputs values, only keys.** |
| [dep-inheritance](../security/dep-inheritance/) | /dep-inheritance | Senior | Every dependency: purpose, risk, replaceability, exit plan. Distinguishes deep transitive risk from direct declarations. |
