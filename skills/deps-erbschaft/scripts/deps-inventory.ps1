...
if (Test-Path -LiteralPath $pyprojectPath) {
    ...
    # Best-effort TOML-Zeilen-Scan (Simplicity First, kein voller TOML-Parser): Zeilen
    # der Form  name = "version"  innerhalb von [...dependencies]-Abschnitten.
    $inDeps = $false
...
if ($manifestsFound.Count -eq 0) {
    Write-Error "Kein unterstuetztes Manifest gefunden (package.json, requirements.txt, pyproject.toml, Cargo.toml, go.mod) in: $root"
    exit 1
}
...
# --- 2. Lockfile-Zaehlung (nur Zaehlwert, keine Tiefenanalyse) ---
$transitiveCount = 0
...
if (Test-Path -LiteralPath $lockNpm) {
    # ConvertFrom-Json scheitert unter PowerShell 5.1 an npm-Lockfile-v3: der
    # Root-Package-Eintrag in "packages" hat den leeren String "" als Key, und
    # PSCustomObject erlaubt keine leeren Property-Namen. JavaScriptSerializer
    # liefert stattdessen ein Hashtable/Dictionary, das damit klarkommt.
